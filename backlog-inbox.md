...
<task_item>
  ...
  <researcher_notes>Numbered TSK-005, not TSK-004: at the time this was drafted, open PR #9 (branch claude/gmail-connection-3vlefm, not yet merged as of 2026-07-11) already claims TSK-004 in its own backlog-inbox.md for an unrelated item ("Add a runnable squad-harness validation tool to the repo" — tools/validate-harness.js). Whichever of PR #9 or this branch merges second must rebase past the other's task_item to avoid a duplicate ID; flagging here so it isn't missed. This repo (the origin of the squad-harness convention) already had the two hardest mechanisms — crash-proof parsing and the 3-strikes circuit breaker — that the newer sibling repos (FreeLanz, driver-log) never picked up. The gap ran the other way: this repo was missing FreeLanz's cost-discipline tracking. See squad-harness/MIGRATION.md in gritui/my-ai-crew for the full per-repo comparison. Reuse Ledger backfilled with the gritui/my-ai-crew spinout, whose README incorrectly claimed to have inherited apps-script/, desktop-app/, and tools/validate-harness.js from this repo as already-integrated files. apps-script/ and desktop-app/ genuinely don't exist in my-ai-crew's repo. tools/validate-harness.js is real, but — as of 2026-07-11 — only exists in the still-open, unmerged PR #9, not on this repo's default branch; corrected 2026-07-11 to state that precisely rather than "never existed," which this task_item's earlier draft got wrong.</researcher_notes>
</task_item>
