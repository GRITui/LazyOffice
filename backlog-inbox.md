...
<task_item>
  ...
  <status>IN_PROGRESS</status>
  <priority>MEDIUM</priority>
  <title>Microsoft Office Add-in prototype — third parallel track, forked to its own branch</title>
  <description>Office Add-in (Word/Excel/PowerPoint) living in office-addin/ on the claude/office-plugin branch, forked from the desktop-app state rather than added alongside it on the same branch. Runs as a task pane (Office.js) inside the host application instead of as a standalone app: no Node/Electron access and no filesystem access from the client, so a local Express server (server/) plays the same role desktop-app/main.js's IPC bridge did, and the task pane inserts content directly into the open document (Word.run/Excel.run) instead of writing a file. PowerPoint has no equivalent direct-write API, so for presentations the server still builds a real .pptx in memory (reusing desktop-app/docgen.js's exact pptxgenjs corporate-theme writer) and hands it to the task pane as base64 to merge in via insertSlidesFromBase64. The output-type selector from the desktop app is replaced by host detection (Office.onReady) — the add-in locks to whichever format matches the document it's already running inside.</description>
  <researcher_notes>Owner decision (2026-07-09): fork to a new branch (claude/office-plugin) rather than a same-branch parallel folder, since this is a distinct distribution/architecture track, not an incremental addition to desktop-app. engine.js/content.js logic-tested the same way as the other tracks (stand-in Claude server, both direct function calls and real HTTP routes through Express), including a genuinely valid .pptx with a real chart part built entirely in memory. Not verified: no real Office application exists in this sandbox, so the manifest has never been sideloaded and Word.run/Excel.run/PowerPoint.run have never executed against a live document. context.presentation.insertSlidesFromBase64 (the PowerPoint insertion call) is the least certain piece — written from documented API shape, flagged in code and README as needing verification against the live Office.js reference during real sideloading.</researcher_notes>
</task_item>
